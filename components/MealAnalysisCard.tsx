...
import { Results, Scene, Mode } from '@/types';
...
export default function MealAnalysisCard({ results, mode, scene }: MealAnalysisCardProps) {
  const { confidence, mood, intent_summary } = results;
  const { messages } = useLanguage();

  const mealIntent =
    scene === 'home'
      ? results?.recipe_strategy?.preferred_cuisine
      : results?.restaurant_strategy?.preferred_cuisine;

  const followUpQuestions = results.follow_up_questions ?? [];
  ...
  return (
    <>
      ...
      <div className="p-4 border border-purple-300 rounded-xl bg-blue-50 mb-4">
        ...
        <div>
          <h3 className="mb-2 text-start">{messages.analysisCard.mealIntent}</h3>
          <div className="flex flex-wrap gap-2">
      ...
      {/* ❓ 低信心補問 */}
      {confidence < 0.6 && followUpQuestions?.length > 0 && mode === 'prompt' && (
        <div className="p-4 border-l-4 border-yellow-400 bg-yellow-50 rounded mb-6">
